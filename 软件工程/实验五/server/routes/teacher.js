/*
 * @Author: zhangl
 * @Date: 2020-05-24 14:47:26
 * @LastEditors: zhangl
 * @LastEditTime: 2020-06-05 13:24:29
 * @FilePath: /server/routes/teacher.js
 * @Description: 老师模块
 */
...
function List(req, res, next) {
    ...
    let SELECT_STUDENT = `
        ...
        AND users.users_id IN (
            ...
            WHERE role_id IN (
                SELECT role_id
                FROM role
                WHERE role_title = 'teacher'
            )
        )
    `;
    const { scene, page, size = 10, tId, search } = req.query;
    if (search) {
        SELECT_STUDENT += `
    ...
    if (tId) {
        SELECT_STUDENT += `
            AND users.users_id = '${xss(escape(tId))}'
        `;
    }
...
function Del(req, res, next) {
    const { id } = req.params;
    if (!id) return res.json(new ErrorModal("老师不存在"));
    ...
    execute(SELECT_STUDENT).then(values => {
        if (!values.length) return res.json(new ErrorModal("老师不存在"));
...
function Add(req, res, next) {
    ...
    const INSERT_STUDENT = `
        INSERT INTO users(users_id, users_name, users_gender, users_age, users_avatar, users_tel, users_email)
        VALUES ('${reqBody.no}','${reqBody.username}','${reqBody.gender}',${reqBody.age},'${reqBody.avatar}','${reqBody.tel}','${reqBody.email}');
    `;
    ...
    const INSERT_USERS_ROLE = `
        ...
        SELECT '${reqBody.no}', (SELECT role_id FROM role where role_title = 'teacher');
    `;
    const SELECT_STUDENT = `SELECT * FROM users WHERE users_id = '${xss(escape(reqBody.no))}'`;
    execute(SELECT_STUDENT).then(values => {
        if (values.length) return res.json(new ErrorModal("老师已存在"));
...
function Detail(req, res, next) {
    ...
    Promise.all([
        ...
        if (!user.length) return res.json(new ErrorModal("老师不存在"));
        const [userRow] = user;
